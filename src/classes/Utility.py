...
[+] Select option: """ + colorText.END))
    ...
    def getNiftyModel(proxyServer=None):
        if os.path.isfile('nifty_model.pkl'):
            file_age = (time.time() - os.path.getmtime('nifty_model.pkl'))/604800
            if file_age > 1:
                download = True
                os.remove('nifty_model.pkl')
            else:
                download = False
        else:
            download = True
        if download:
            file_url = "https://raw.github.com/pranjal-joshi/Screeni-py/new-features/src/ml/nifty_model.pkl"
            if proxyServer is not None:
