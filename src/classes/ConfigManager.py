...
class tools:
    ...
    def setConfig(parser, default=False):
        if default:
            ...
            daysToLookback = input('[+] Number of recent days (TimeFrame) to screen for Breakout/Consolidation (Days)(Optimal = 20): ')
            ...
            useEmaPrompt = str(input('[+] Use EMA instead of SMA? (EMA is good for Short-term & SMA for Mid/Long-term trades)[Y/N]: ')).lower()
    ...
    def getConfig(parser):
        ...
        if len(parser.read('screenipy.ini')):
            try:
                ...
                input(colorText.BOLD + colorText.FAIL +'[+] Screenipy requires user configuration again. Press enter to continue..' + colorText.END)
